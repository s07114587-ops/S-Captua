...
(function () {
  ...
  var css = `
  :root{ --bg:#000000; --bg-soft:#0a0a0a; --panel:rgba(10,10,10,0.95); --border:rgba(255,255,255,0.15); --sca-accent:#10b981; --sca-glow:rgba(16,185,129,0.45); --text:#ffffff; --text-dim:#9ca3af; --danger:#f43f5e; --radius:14px;
  } .scaptcha-auto-wrapper{ display:flex; justify-content:center; align-items:center; width:100%; margin:10px 0; } .scaptcha{ width:320px; background:var(--panel); border:1.5px solid var(--sca-accent);
  border-radius:var(--radius); backdrop-filter:blur(14px); -webkit-backdrop-filter:blur(14px); box-shadow:0 0 18px var(--sca-glow), 0 8px 32px rgba(0,0,0,0.5); overflow:hidden; user-select:none; font-family:'Segoe UI',system-ui,-apple-system,sans-serif; color:var(--text); animation:scNeonCycle 6s linear infinite; }
  ...
  cursor:pointer; transition:all .2s ease; position:relative; } .sc-box:hover{ border-color:var(--sca-accent); } .sc-box:focus-visible{ outline:2px solid var(--sca-accent); outline-offset:2px; }
  .sc-box.checked{ border-color:var(--sca-accent); background:var(--sca-accent); box-shadow:0 0 14px var(--sca-glow); } .sc-box.failed{ border-color:var(--danger); animation:scShake .35s ease; } .sc-box svg{ width:16px; height:16px; stroke:#0f172a; stroke-width:3; fill:none; stroke-linecap:round; stroke-linejoin:round; opacity:0; transform:scale(.5); transition:all .15s ease; } .sc-box.checked svg{ opacity:1; transform:scale(1); }
  .sc-spinner{ width:18px; height:18px; border-radius:50%; border:2.5px solid rgba(16,185,129,0.25); border-top-color:var(--sca-accent); animation:scSpin .7s linear infinite; display:none; } .sc-box.loading .sc-spinner{ display:block; } .sc-box.loading svg{ display:none; } .sc-label{ flex:1; cursor:pointer; }
  .sc-label-text{ font-size:14.5px; font-weight:500; color:var(--text); line-height:1.2; } .sc-sub{ font-size:11px; color:var(--text-dim); margin-top:2px; transition:color .2s ease; } .sc-sub.ok{ color:var(--sca-accent); } .sc-sub.err{ color:var(--danger); }
  ...
  .sc-modal{ width:320px; background:linear-gradient(180deg, rgba(15,15,15,0.98), rgba(0,0,0,0.99)); border:1.5px solid var(--sca-accent); border-radius:var(--radius); box-shadow:0 0 24px var(--sca-glow), 0 20px 60px rgba(0,0,0,0.7); padding:16px; font-family:'Segoe UI',sans-serif; animation:scNeonCycle 6s linear infinite; animation-delay:-2s; } .sc-modal.sc-modal-wide{ width:360px; } .sc-modal-head{ display:flex; align-items:center; justify-content:space-between; margin-bottom:10px; color:var(--text); } .sc-modal-head h3{ margin:0; font-size:13.5px; }
  ...
  } .sc-grid-tile:hover{ border-color:rgba(255,255,255,0.25); } .sc-grid-tile:focus-visible{ outline:2px solid var(--sca-accent); outline-offset:2px; } .sc-grid-tile.sc-tile-correct{ border-color:var(--sca-accent); background:rgba(16,185,129,0.18); }
  ...
  .sc-captcha-row{ display:flex; gap:8px; width:100%; } .sc-captcha-input{ flex:1; background:rgba(255,255,255,0.04); border:1px solid var(--border); border-radius:8px; padding:9px 11px; color:var(--text); font-size:14px; letter-spacing:2px; text-transform:uppercase; } .sc-captcha-input:focus{ outline:2px solid var(--sca-accent); outline-offset:1px; } .sc-captcha-refresh{ flex:0 0 auto; width:38px; border-radius:8px; border:1px solid var(--border); background:rgba(255,255,255,0.04); color:var(--text-dim); font-size:16px; cursor:pointer; }
  .sc-captcha-refresh:hover{ color:var(--sca-accent); border-color:var(--sca-accent); } .sc-captcha-submit{ width:100%; padding:10px; border-radius:8px; border:none; background:var(--sca-accent); color:#0f172a; font-weight:700; font-size:13.5px; cursor:pointer; } .sc-captcha-submit:hover{ box-shadow:0 0 14px var(--sca-glow); } .sc-captcha-err{ color:var(--danger); font-size:11.5px; margin-top:-2px; }
  ...
  @keyframes scNeonCycle{ 0%{ border-color:#39FF14; box-shadow:0 0 20px rgba(57,255,20,0.65); } 14%{ border-color:#FF10F0; box-shadow:0 0 20px rgba(255,16,240,0.65); } 28%{ border-color:#00FFFF; box-shadow:0 0 20px rgba(0,255,255,0.65); } 42%{ border-color:#BF00FF; box-shadow:0 0 20px rgba(191,0,255,0.65); } 56%{ border-color:#FFF01F; box-shadow:0 0 20px rgba(255,240,31,0.65); } 70%{ border-color:#FF6B00; box-shadow:0 0 20px rgba(255,107,0,0.65); } 84%{ border-color:#14FFEC; box-shadow:0 0 20px rgba(20,255,236,0.65); } 100%{ border-color:#39FF14; box-shadow:0 0 20px rgba(57,255,20,0.65); } }
  `;
  var styleTag = document.createElement("style");
  styleTag.setAttribute("data-scaptcha", "1");
  styleTag.textContent = css;
  document.head.appendChild(styleTag);
  // Random neon accent every page load. Namespaced --sca-* vars (not
  // --accent/--accent-glow) so this never collides with the host page's
  // own CSS variables, and an inline style on <html> beats the stylesheet
  // :root rule's specificity so it actually overrides the default green.
  (function applyRandomNeon() {
    var NEON = ["#39FF14", "#FF10F0", "#00FFFF", "#BF00FF", "#FF6B00", "#FFF01F", "#00F0FF", "#FF073A", "#D4FF00", "#14FFEC", "#FF00A8", "#7CFC00"];
    var pick = NEON[Math.floor(Math.random() * NEON.length)];
    var h = pick.replace("#", "");
    var r = parseInt(h.substring(0, 2), 16), g = parseInt(h.substring(2, 4), 16), b = parseInt(h.substring(4, 6), 16);
    var root = document.documentElement.style;
    root.setProperty("--sca-accent", pick);
    root.setProperty("--sca-glow", "rgba(" + r + "," + g + "," + b + ",0.55)");
  })();
