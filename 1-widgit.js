...
(function () {
  ...
  var css = `
  :root{ --bg:#f8fafc; --bg-soft:#eef2f7; --panel:rgba(255,255,255,0.96); --border:rgba(15,23,42,0.12); --accent:#10b981; --accent-glow:rgba(16,185,129,0.35); --text:#0f172a; --text-dim:#64748b; --danger:#e11d48; --radius:14px;
  ...
  .sc-modal{ width:320px; background:linear-gradient(180deg, rgba(255,255,255,0.98), rgba(248,250,252,0.98)); border:1px solid var(--border); border-radius:var(--radius); box-shadow:0 20px 60px rgba(15,23,42,0.18); padding:16px; font-family:'Segoe UI',sans-serif; } .sc-modal.sc-modal-wide{ width:360px; } .sc-modal-head{ display:flex; align-items:center; justify-content:space-between; margin-bottom:10px; color:var(--text); } .sc-modal-head h3{ margin:0; font-size:13.5px; }
  ...
  function drawCaptchaCanvas(canvas, text) {
    ...
    // noise dots
    for (var d = 0; d < 130; d++) {
      ctx.fillStyle = "rgba(16,185,129," + (0.12 + Math.random() * 0.25) + ")";
      ctx.beginPath();
    ...
    for (var idx = 0; idx < text.length; idx++) {
      ...
      ctx.fillStyle = "hsl(" + (150 + Math.random() * 40) + ",70%," + (28 + Math.random() * 12) + "%)";
